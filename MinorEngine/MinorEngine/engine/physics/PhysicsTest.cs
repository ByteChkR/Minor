...
namespace GameEngine.engine.physics
{
    ...
    public static class SimpleSelfContainedDemo
    {
        ...
        public static void Init()
        {
            ...
            simulation = Simulation.Create(bufferPool, new NarrowPhaseCallbacks(), new PoseIntegratorCallbacks(new Vector3(0, -10, 0)));
        ...
        internal static BodyReference AddBoxDynamic(float mass, Vector3 position, Vector3 dimensions)
        {
            ...
            return simulation.Bodies.GetBodyReference(simulation.Bodies.Add(
                BodyDescription.CreateDynamic(position, boxIntertia,
                    new CollidableDescription(simulation.Shapes.Add(b), 0.1f),
                    new BodyActivityDescription(0.01f))));
        }

        internal static void AddBoxStatic(Vector3 position, Vector3 dimensions)
        {
            simulation.Statics.Add(new StaticDescription(position, new CollidableDescription(simulation.Shapes.Add(new Box(dimensions.X, dimensions.Y, dimensions.Z)), 0.1f)));
        }
        ...
        internal static BodyReference AddSphereDynamic(float mass, Vector3 position, float radius)
        {
            ...
            return simulation.Bodies.GetBodyReference(simulation.Bodies.Add(
                BodyDescription.CreateDynamic(position, sphereInertia,
                    new CollidableDescription(simulation.Shapes.Add(sphere), 0.1f),
                    new BodyActivityDescription(0.01f))));

        }

        internal static void AddBoxStatic(Vector3 position, float radius)
        {
            simulation.Statics.Add(new StaticDescription(position, new CollidableDescription(simulation.Shapes.Add(new Sphere(radius)), 0.1f)));
        }
    }
}