...
namespace MinorEngine.engine.physics
{
    public class ThreadDispatcher : IThreadDispatcher, IDisposable
    {
        ...
        public ThreadDispatcher(int threadCount)
        {
            ...
            for (int i = 0; i < _bufferPools.Length; ++i) _bufferPools[i] = new BufferPool();
        }
        ...
        void DispatchThread(int workerIndex)
        {
            ...
            if (Interlocked.Increment(ref _completedWorkerCounter) == _threadCount) _finished.Set();
        }
        ...
        void SignalThreads()
        {
            for (int i = 0; i < _workers.Length; ++i) _workers[i].Signal.Set();
        }
        ...
        public void Dispose()
        {
            if (!disposed)
            {
                disposed = true;
                SignalThreads();
                for (int i = 0; i < _bufferPools.Length; ++i) _bufferPools[i].Clear();
